...
const verifyAccount = (verificationCode) => {
    return new Promise(async (resolve, reject) => {
      ...
      `;
            ...
            mySqlConnection.query(sqlQuery, values, async (error, results) => {
                if (error) {
                    console.log(error);
                }
                else if (results.length > 0) {
                    const { id, username, email, hashed_password, creation_time } = results[0];
                    await saveUserDataToVerifiedDatabase(id, username, email, hashed_password, creation_time);
